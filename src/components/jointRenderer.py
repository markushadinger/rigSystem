...
class JointRenderer(MacroComponent):
    in_mtxs = DeferredPlug("input", "input", MATRIX, multi=True)
    ...
    def build(self):
        for i, index in enumerate(self.indices):
            ...
            jnt.offsetParentMatrix.connect(self.in_mtxs.plug[i])

            if self.from_guides:
                cmds.xform(jnt, worldSpace=True, matrix=guide.get_world_matrix(self.name.replace(index=index)))
            else:
                cmds.xform(jnt, ws=False, m=guide.DEFAULT_VALUE)
